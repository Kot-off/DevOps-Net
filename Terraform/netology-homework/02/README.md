...
## Задание 5: Динамическое формирование имен (Interpolation)

Для формирования имен ресурсов применен механизм интерполяции строк, что позволяет избежать «хардкода» и сделать конфигурацию гибкой.

* **Решение**: В файле `locals.tf` реализована сборка имен ВМ из нескольких составляющих переменных: `${var.project}-${var.vpc_name}-${var.platform}-web/db`.
* **Техническая ценность**: Использование синтаксиса `${...}` позволяет автоматически обновлять имена всех зависимых ресурсов при изменении одной глобальной переменной (например, при смене окружения с `develop` на `production`).
* [Документация: String Interpolation](https://www.google.com/search?q=https://developer.hashicorp.com/terraform/language/expressions/strings%23interpolation)
