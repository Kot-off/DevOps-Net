...
1. **Задание 7 (Фильтрация)**: Из объекта `local.vpc` удален 3-й элемент списков с помощью конструкции:
```hcl
...
2. **Задание 8 (Исправление ошибок)**: Устранена ошибка обращения к вложенным структурам в шаблоне `.tftpl` (исправлена логика итерации по списку объектов).
...
1. **Задание 9 (Генерация имен)**: Создан генератор имен серверов `rc01-rc99` с пропуском каждого десятого имени:
```hcl
